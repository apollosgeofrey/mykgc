...
<div class="col-sm-12">
    <div class="card">
        ...
        //end
                                ...
                                if (isset($conn)) {
                                    $query_hist = $conn->prepare("SELECT * FROM `kokozum_payment_records` WHERE payment_status='0' $id_next ORDER BY db_id DESC LIMIT 15");
                                    $query_hist->execute();
                                    ...
                                    if ($query_hist_run == true && mysqli_num_rows($query_hist_run) >= 1) {
                                      while ($rows = mysqli_fetch_assoc($query_hist_run)) {
                                        ...
                                        echo "
                                        <tr>
                                            <td><a href='taxinvoices.php?db_db_id=$db_db_id&tx_refer=$db_tx_ref'>$db_tx_ref</a></td>
                                            <td>N $db_amount_paid $db_payment_status</td>
                                            <td><a href='taxinvoices.php?db_db_id=$db_db_id&tx_refer=$db_tx_ref'>$db_local_grabed_date_time</a></td>
                                        </tr>";
                                        ...
                                        if ($id_val < $db_db_id) {
                                            ...
                                            $id_val = $db_db_id + 1;
                                           $see_older_disable = "<li class='pull-right btn btn-sm'><a href='invoice_history.php?id_next=$id_val&id_newer=$id_val'>See older</a></li>";
                                          }
